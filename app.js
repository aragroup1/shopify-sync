...
function processApifyProducts(apifyData, { processPrice = true } = {}) {
    return apifyData.map(item => {
        ...
        const inventory = stockStatus.toLowerCase() === 'instock' ? 20 : 0; // Default to 20 if in stock
        ...
        let price = '0.00';
        const supplierCostPence = item.variants?.[0]?.price?.current;
        if (supplierCostPence !== undefined && supplierCostPence !== null) {
            const supplierCost = parseFloat(supplierCostPence) / 100; // Convert pence to pounds
            if (!isNaN(supplierCost)) {
                price = processPrice ? calculateRetailPrice(String(supplierCost)) : supplierCost.toFixed(2);
            }
        }
        ...
        if (Array.isArray(item.medias)) {
            images = item.medias
                .map(mediaObj => (mediaObj && mediaObj.url ? { src: mediaObj.url } : null))
