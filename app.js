...
function processApifyProducts(apifyData, { processPrice = true } = {}) {
    return apifyData.map(item => {
        ...
        // Inventory Logic based on stockStatus
        const stockStatus = item.variants?.[0]?.price?.stockStatus || '';
        const inventory = stockStatus.toLowerCase() === 'instock' ? 20 : 0; // Default to 20 if in stock, 0 otherwise
        ...
        let price = '0.00';
        const supplierCostInPence = item.variants?.[0]?.price?.current;
        if (supplierCostInPence !== undefined && supplierCostInPence !== null) {
            // Convert pence to pounds for calculation
            const supplierCostInPounds = parseFloat(supplierCostInPence) / 100;
            if (!isNaN(supplierCostInPounds)) {
                // Use the cost in pounds for pricing logic
                price = processPrice ? calculateRetailPrice(String(supplierCostInPounds)) : supplierCostInPounds.toFixed(2);
            }
        }
        ...
        if (Array.isArray(item.medias)) {
            images = item.medias
                .map(mediaObject => (mediaObject && mediaObject.url ? { src: mediaObject.url } : null))
                .filter(img => img); // Filter out any nulls or invalid entries
        }
    ...
    }).filter(p => p && p.sku); // Ensure we only return products that have a SKU
}
