...
// CRITICAL FIX: Get actual inventory levels from Shopify's Inventory API
async function getActualInventoryLevels(inventoryItemIds) {
  ...
  for (let i = 0; i < inventoryItemIds.length; i += batchSize) {
    ...
    try {
      ...
      addLog(`Failed to fetch inventory levels batch: ${error.message}`, 'error', 'inventory');
    }
...
// CRITICAL FIX: Complete rewrite of inventory update to use actual inventory levels
async function updateInventoryJob(token) {
  ...
  try {
    ...
    // CRITICAL: Get actual inventory levels from the Inventory API
    ...
    processedProducts.forEach((apifyProduct, idx) => {
      ...
      // CRITICAL: Use actual inventory level from the API, not from variant
    ...
    // First, connect products not at location
    ...
    addToHistory('inventory', { updated, errors, attempted: inventoryUpdates.length + notConnectedToLocation.length, trackingEnabled, skipped });
    ...
    if (repeatUpdates.length > 50) {
      await notifyTelegram(`⚠️ Inventory Sync Issue Detected!\n\n${repeatUpdates.length} products keep needing updates after being updated.\n\nThis run connected ${notConnectedToLocation.length} products to the location which may resolve the issue.\n\nPlease run another sync to verify.`);
    }
...
app.listen(PORT, () => {
  ...
  addLog('CRITICAL FIX: Now fetching actual inventory levels from Shopify Inventory API', 'info', 'system');
