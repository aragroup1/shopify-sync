...
function cleanProductTitle(title) {
    // Remove SKUs (4+ digit numbers)
    title = title.replace(/\b\d{4,}\b/g, '').trim();
    
    // Remove anything in brackets including the brackets
    title = title.replace(/KATEX_INLINE_OPEN[^)]*KATEX_INLINE_CLOSE/g, '').trim();
...
async function deduplicateProductsJob(token) { 
    ...
    let deletedCount = 0, errors = 0, duplicateGroups = 0; 
        ...
        for (const products of productsByTitle.values()) { 
            if (products.length > 1) { 
                ...
                products.forEach(p => { 
                    ...
                    deletedProducts.push({ id: p.id, title: p.title });
                }); 
            } 
        } 
        
        if (toDeleteIds.length > 0) { 
            addLog(`Found ${duplicateGroups} groups of duplicates. Preparing to delete ${toDeleteIds.length} newer duplicate products...`, 'warning'); 
