...
// --- DIAGNOSTIC FUNCTION ---
// This version will log the raw data for the first 5 items from Apify.
let logCount = 0;
function processApifyProducts(apifyData, { processPrice = true } = {}) {
    return apifyData.map(item => {
        // --- TEMPORARY LOGGING ---
        if (logCount < 5) {
            console.log(`--- RAW APIFY ITEM [${logCount + 1}] ---`);
        ...
        let price = '0.00';
        const priceSource = item.variants?.[0]?.price?.value || item.variants?.[0]?.price || item.price?.value || item.price;
        if (priceSource) {
            const priceString = typeof priceSource === 'object' ? String(priceSource.value) : String(priceSource);
            price = processPrice ? calculateRetailPrice(priceString) : parseFloat(priceString).toFixed(2);
        }
...
// --- CORE JOB LOGIC ---
// ... (All other job functions remain exactly the same as the previous version)
...
async function createNewProductsJob(token, { overrideFailsafe = false } = {}) {
    logCount = 0; // Reset diagnostic log counter
...
// --- UI AND API ---
// ... (The UI and API sections are identical to the previous version and do not need to be changed)
app.get('/', (req, res) => {
