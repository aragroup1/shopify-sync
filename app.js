...
// CLEAN TITLE FUNCTION - Removes parcel rates and SKUs
function cleanProductTitle(title) {
  if (!title) return '';
  
  // Remove anything in parentheses (parcel rates, etc.)
  let cleaned = title.replace(/KATEX_INLINE_OPEN[^)]*KATEX_INLINE_CLOSE/g, '');
  
  // Remove SKU patterns like SK28659, ST80056, DE-8335C, R38864
  cleaned = cleaned.replace(/\b[A-Z]{1,3}[-]?\d{4,}[A-Z]?\b/gi, '');
  
  // Remove standalone 4+ digit numbers
...
function normalizeForMatching(text = '') { return String(text).toLowerCase().replace(/KATEX_INLINE_OPEN[^)]*KATEX_INLINE_CLOSE/g, ' ').replace(/```math.*?```/g, ' ').replace(/```math.*?```/gs, ' ').replace(/-parcel-large-letter-rate$/i, '').replace(/-p\d+$/i, '').replace(/\b(a|an|the|of|in|on|at|to|for|with|by)\b/g, '').replace(/[^a-z0-9]+/g, ' ').replace(/\s+/g, ' ').trim(); }
...
function processApifyProducts(apifyData, { processPrice = true } = {}) {
    return apifyData.map(item => {
        ...
        // Clean the title here as well for storage
    ...
    try {
        ...
        if (toCreate.length > 0) {
            ...
            for (let i = 0; i < maxToCreate; i++) {
                ...
                // Use cleanedTitle if available, otherwise clean the title now
                const cleanedTitle = apifyProd.cleanedTitle || cleanProductTitle(apifyProd.title);
                
                // Log the cleaning for debugging
                if (apifyProd.title !== cleanedTitle) {
                    addLog(`Title cleaned: "${apifyProd.title}" -> "${cleanedTitle}"`, 'info');
                }
