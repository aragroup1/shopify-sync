...
const config = { 
  apify: { token: process.env.APIFY_TOKEN, actorId: process.env.APIFY_ACTOR_ID || 'autofacts~shopify', baseUrl: 'https://api.apify.com/v2', urlPrefix: process.env.URL_PREFIX || 'https://www.manchesterwholesale.co.uk/products/' }, 
...
function startBackgroundJob(key, name, fn) {
  ...
  setImmediate(async () => {
    try { await fn(token); } catch (e) { addLog(`Unhandled error in ${name}: ${e.message}`, 'error'); } finally { jobLocks[key] = false; addLog(`${name} job finished`, 'info'); }
  });
  return true;
}
...
async function getShopifyProducts({ onlyApifyTag = true, fields = 'id,handle,title,variants,tags,status' } = {}) {
    ...
    } catch (error) { addLog(`Shopify fetch error: ${error.message}`, 'error'); triggerFailsafe(`Shopify fetch failed: ${error.message}`); throw error; }
    ...
    for (let i = 0; i < inventoryItemIds.length; i += batchSize) {
        ...
        while (!success && retries < maxRetries) {
            try {
                ...
                } else { addLog(`Batch fetch failed: ${error.message}`, 'error'); break; }
            }
        }
        if (!success) { addLog(`Batch failed after ${maxRetries} retries. Skipping.`, 'error'); }
...
function processApifyProducts(apifyData, options = { processPrice: true }) {
    return apifyData.map((item, index) => {
        ...
        return { handle, title: item.title, inventory, sku: item.sku, price };
    }).filter(Boolean);
}
...
function buildShopifyMaps(shopifyData) {
    ...
    shopifyData.forEach(product => {
        ...
        if (sku && sku.trim() !== '') skuMap.set(sku.toLowerCase(), product);
    });
    return { handleMap, titleMap, skuMap };
}

/**
 * Matches an Apify product by finding the BEST Shopify product match.
 * It prioritizes SKU, then finds the closest handle prefix match by length,
 * preventing greedy, incorrect matches.
 * @param {object} apifyProduct - The processed product data from Apify.
 * @param {object} maps - The maps of Shopify products (handleMap, titleMap, skuMap).
 * @returns {object} - { product, matchType } or { product: null, matchType: 'none' }.
 */
function flexibleMatchShopifyProduct(apifyProduct, maps) {
  // 1. Prioritize SKU match (most reliable)
  if (apifyProduct.sku && apifyProduct.sku.trim() !== '') {
    const bySku = maps.skuMap.get(apifyProduct.sku.toLowerCase());
    if (bySku) return { product: bySku, matchType: 'sku' };
  }

  // 2. Find the BEST Handle Prefix Match
  const apifyHandle = apifyProduct.handle.toLowerCase();
  let bestMatch = null;
  let minLengthDiff = Infinity;

  // Find all potential matches and select the one with the shortest handle "tail"
  for (const [shopifyHandle, shopifyProduct] of maps.handleMap.entries()) {
    if (shopifyHandle.startsWith(apifyHandle)) {
      const diff = shopifyHandle.length - apifyHandle.length;
  ...
  if (bestMatch) {
    return { product: bestMatch, matchType: 'handle_prefix_best' };
  }

  // 3. Normalized Title Match (last resort fallback)
...
// --- CORE JOB LOGIC ---
async function fixInventoryTrackingJob(token) { /* ... implementation from before ... */ }
async function executeInventoryUpdates(updates, token) { /* ... implementation from before ... */ }
...
async function updateInventoryJob(token) {
  ...
  try {
    ...
    if (shouldAbort(token) || apifyData.length === 0) {
        addLog('Aborting inventory sync (system paused or no Apify products).', 'warning');
        return;
    }
    ...
    processedProducts.forEach((apifyProduct) => {
      const { product: shopifyProduct, matchType } = flexibleMatchShopifyProduct(apifyProduct, maps);
      ...
      let currentInventory = inventoryLevels.get(inventoryItemId);
      ...
      addLog(`[${matchType}] Mismatch for "${shopifyProduct.title}": Shopify=${currentInventory}, Apify=${targetInventory}. Queuing update.`, 'warning');
...
async function handleDiscontinuedProductsJob(token) {
    ...
    try {
        ...
        apifyProcessed.forEach(p => { const { product } = flexibleMatchShopifyProduct(p, shopifyMaps); if (product) matchedShopifyIds.add(product.id); });
        
        const candidates = shopifyData.filter(p => !matchedShopifyIds.has(p.id));
        const nowMissing = [];
        for (const p of candidates) {
            const count = (missingCounters.get(p.handle.toLowerCase()) || 0) + 1;
            missingCounters.set(p.handle.toLowerCase(), count);
        ...
        for (const product of nowMissing) {
            ...
            try {
                if (product.variants?.[0]?.inventory_item_id) { await shopifyClient.post('/inventory_levels/set.json', { location_id: parseInt(config.shopify.locationId), inventory_item_id: product.variants[0].inventory_item_id, available: 0 }); }
            ...
            } catch (e) { errors++; stats.errors++; addLog(`Error discontinuing "${product.title}": ${e.message}`, 'error'); }
            await new Promise(r => setTimeout(r, 500));
        }
...
async function createNewProductsJob(token) {
  ...
  try {
    ...
    const shopifyMaps = buildShopifyMaps(shopifyData);
    const toCreate = apifyProcessed.filter(apifyProduct => {
        const { product: existingProduct } = flexibleMatchShopifyProduct(apifyProduct, shopifyMaps);
        return !existingProduct;
    });
    
    addLog(`Found ${toCreate.length} products to create`, 'info');
    const limitedToCreate = toCreate.slice(0, MAX_CREATE_PER_RUN);
    if (limitedToCreate.length > 0) addLog(`Creating ${limitedToCreate.length} products (limited to ${MAX_CREATE_PER_RUN} per run)`, 'info');
    
    for (const product of limitedToCreate) {
      if (shouldAbort(token)) break;
      try {
        const shopifyProduct = { title: product.title, handle: product.handle, vendor: 'Manchester Wholesale', tags: 'Supplier:Apify', status: 'active', variants: [{ price: product.price || '0.00', sku: product.sku || '', inventory_management: 'shopify', inventory_quantity: product.inventory }] };
        ...
        if (response.data.product?.variants?.[0]?.inventory_item_id) {
          ...
          await shopifyClient.post('/inventory_levels/connect.json', { location_id: parseInt(config.shopify.locationId), inventory_item_id: invItemId }).catch(e => { if (e.response?.status !== 422) addLog(`Warning: Could not connect inventory for "${product.title}"`, 'warning'); });
        ...
        created++; stats.newProducts++;
        addLog(`✓ Created: "${product.title}" with ${product.inventory} stock`, 'success');
      } catch (error) { errors++; stats.errors++; addLog(`✗ Failed to create "${product.title}": ${error.response?.data?.errors ? JSON.stringify(error.response.data.errors) : error.message}`, 'error'); }
      await new Promise(r => setTimeout(r, 1000));
    }
  } catch (error) { addLog(`Product creation job failed: ${error.message}`, 'error'); errors++; }
...
// --- UI AND API, CRON JOBS, AND SERVER START ---
// This entire section remains unchanged from your last version.
// ...
app.get('/', (req, res) => {
  let failsafeBanner = '';
...
app.post('/api/fix/inventory-tracking', (req, res) => { startBackgroundJob('fixTracking', 'Fix Inventory Tracking', async (t) => fixInventoryTrackingJob(t)) ? res.json({success: true, message: "Fix job started."}) : res.status(409).json({success: false, message: "Fix job already running."}); });
app.post('/api/sync/inventory', (req, res) => { startBackgroundJob('inventory', 'Manual Inventory Sync', async (t) => updateInventoryJob(t)) ? res.json({success: true, message: "Sync started."}) : res.status(409).json({success: false, message: "Sync already running."}); });
app.post('/api/sync/products', (req, res) => { startBackgroundJob('products', 'Manual Product Sync', async (t) => createNewProductsJob(t)) ? res.json({success: true, message: "Create started."}) : res.status(409).json({success: false, message: "Create already running."}); });
app.post('/api/sync/discontinued', (req, res) => { startBackgroundJob('discontinued', 'Manual Discontinued Sync', async (t) => handleDiscontinuedProductsJob(t)) ? res.json({success: true, message: "Discontinued check started."}) : res.status(409).json({success: false, message: "Check already running."}); });
