...
let failsafeReason = '';
let pendingFailsafeAction = null; // Stores the function to run on override
...
function processApifyProducts(apifyData, { processPrice = true } = {}) {
    return apifyData.map(item => {
        ...
        // --- Robust Inventory Logic ---
        let inventory = item.variants?.[0]?.stockQuantity || item.stock || 20;
        if (String(item.variants?.[0]?.price?.stockStatus || item.stockStatus || item.availability || '').toLowerCase().includes('out')) {
            inventory = 0;
        }
        ...
        // --- NEW: More Robust Price Finding Logic ---
        let price = '0.00';
        // Look for price in multiple common locations
        const priceSource = item.variants?.[0]?.price?.value || 
                            item.variants?.[0]?.price || 
                            item.price?.value || 
                            item.price;

        if (priceSource) {
            // Ensure we're dealing with a number-like string, not an object
            const priceString = typeof priceSource === 'object' ? String(priceSource.value) : String(priceSource);
            if (processPrice) {
                price = calculateRetailPrice(priceString);
            } else {
                price = parseFloat(priceString).toFixed(2);
            }
        }
        
        // --- NEW: More Robust Image Finding Logic ---
        let images = [];
        // Look for images in multiple common locations
        const imageSource = item.images || item.imageUrls || (item.image ? [item.image] : []);

        if (Array.isArray(imageSource) && imageSource.length > 0) {
            images = imageSource
                .map(img => {
                    // Handle cases where img is a string URL or an object with src/url property
                    const src = typeof img === 'string' ? img : (img.src || img.url);
                    return src ? { src } : null;
                })
                .filter(img => img); // Filter out any nulls
        }
...
async function updateInventoryJob(token, { overrideFailsafe = false } = {}) {
    ...
    try {
        ...
        if (updates.length > 0) {
            ...
            for (const update of updates) { if (shouldAbort(token)) break; try { await shopifyClient.post('/inventory_levels/set.json', update); addLog(`Updated inventory for "${update.title}" from ${update.current} to ${update.available}`, 'info'); updated++; } catch (e) { errors++; addLog(`Error updating inventory for "${update.title}": ${e.message}`, 'error', e); } await new Promise(r => setTimeout(r, 200)); }
        }
        if (statusUpdates.length > 0) { for (const update of statusUpdates) { if (shouldAbort(token)) break; try { await shopifyClient.put(`/products/${update.id}.json`, { product: { id: update.id, status: update.newStatus } }); addLog(`Updated status for "${update.title}" from ${update.oldStatus} to ${update.newStatus}`, 'success'); statusChanged++; } catch (e) { errors++; addLog(`Error updating status for "${update.title}": ${e.message}`, 'error', e); } await new Promise(r => setTimeout(r, 500)); } }
...
async function createNewProductsJob(token, { overrideFailsafe = false } = {}) {
    ...
    try {
        ...
        if (toCreate.length > 0) {
            ...
            for (let i = 0; i < maxToCreate; i++) { if (shouldAbort(token)) break; const apifyProd = toCreate[i]; try { const newProduct = { product: { title: apifyProd.title, body_html: apifyProd.body_html || '', vendor: 'Imported', tags: SUPPLIER_TAG, status: apifyProd.inventory > 0 ? 'active' : 'draft', variants: [{ price: apifyProd.price, sku: apifyProd.sku, inventory_management: 'shopify' }], images: apifyProd.images } }; const { data } = await shopifyClient.post('/products.json', newProduct); const inventoryItemId = data.product.variants[0].inventory_item_id; await shopifyClient.post('/inventory_levels/set.json', { inventory_item_id: inventoryItemId, location_id: config.shopify.locationId, available: apifyProd.inventory }); created++; createdItems.push({ id: data.product.id, title: apifyProd.title, sku: apifyProd.sku }); addLog(`Created product: "${apifyProd.title}" (SKU: ${apifyProd.sku}, Status: ${newProduct.product.status})`, 'success'); } catch (e) { errors++; addLog(`Error creating product "${apifyProd.title}": ${e.message}`, 'error', e); } await new Promise(r => setTimeout(r, 1000)); }
...
async function handleDiscontinuedProductsJob(token, { overrideFailsafe = false } = {}) {
    ...
    try {
        ...
        const supplierProducts = shopifyData.filter(p => p.tags && p.tags.includes(SUPPLIER_TAG));
        const potentialDiscontinued = supplierProducts.filter(p => { const sku = p.variants?.[0]?.sku?.toLowerCase(); return sku && !apifySkus.has(sku) && p.status === 'active'; });
        addLog(`Found ${potentialDiscontinued.length} potentially discontinued products.`, 'info');
        ...
        if (potentialDiscontinued.length > 0) {
            ...
            for (const product of potentialDiscontinued) { if (shouldAbort(token)) break; try { const inventoryItemId = product.variants[0].inventory_item_id; if (inventoryItemId) { await shopifyClient.post('/inventory_levels/set.json', { inventory_item_id: inventoryItemId, location_id: config.shopify.locationId, available: 0 }); } await shopifyClient.put(`/products/${product.id}.json`, { product: { id: product.id, status: 'draft' } }); discontinued++; discontinuedItems.push({ id: product.id, title: product.title, sku: product.variants[0].sku }); addLog(`Discontinued product: "${product.title}" (SKU: ${product.variants[0].sku})`, 'success'); } catch (e) { errors++; addLog(`Error discontinuing product "${product.title}": ${e.message}`, 'error', e); } await new Promise(r => setTimeout(r, 600)); }
