...
// IMPROVED SKU mapping job that tries multiple matching strategies
// IMPROVED SKU mapping job that only processes products with the supplier tag
async function improvedMapSkusJob(token) { 
  addLog(`--- Starting Comprehensive SKU Mapping Job (for ${SUPPLIER_TAG} products) ---`, 'warning'); 
  ...
  try { 
    ...
    // Filter Shopify products to only include those with the supplier tag or no tag at all
    const supplierProducts = shopifyData.filter(p => {
      if (!p.tags) return true; // Include products with no tags (potential matches)
      return p.tags.includes(SUPPLIER_TAG);
    });
    
    addLog(`Found ${supplierProducts.length} potential ${SUPPLIER_TAG} products out of ${shopifyData.length} total products`, 'info');
    skippedNonSupplier = shopifyData.length - supplierProducts.length;
    
    // First, map products that already have SKUs
    ...
    // Process products without matching SKUs
    const toProcess = supplierProducts.filter(p => !usedShopifyIds.has(p.id));
    addLog(`Processing ${toProcess.length} supplier products that need SKU mapping...`, 'info');
    
    const updateCandidates = [];
    
    // For each product in Shopify without a matching SKU
    for (const shopifyProd of toProcess) {
      ...
      // If no handle match, try title match
      if (!bestMatch) {
        for (const apifyProd of apifyProcessed) {
      ...
      // If we found a match, prepare to update the SKU
      if (bestMatch) {
        ...
        if (shopifyVariant.sku?.toLowerCase() === bestMatch.sku.toLowerCase()) {
          // Already matched but wasn't caught in first pass (case difference?)
          alreadyMatched++;
        } else {
    ...
    addLog(`- Skipped non-supplier products: ${skippedNonSupplier}`, 'info');
    ...
    if (updateCandidates.length > 0) {
      ...
      for (const item of updateCandidates) {
        ...
        try {
          ...
          // Also ensure the product has the supplier tag
          if (!shopifyData.find(p => p.id === item.shopifyId)?.tags?.includes(SUPPLIER_TAG)) {
            const product = shopifyData.find(p => p.id === item.shopifyId);
            if (product) {
              const tags = product.tags ? `${product.tags}, ${SUPPLIER_TAG}` : SUPPLIER_TAG;
              await shopifyClient.put(`/products/${item.shopifyId}.json`, {
                product: {
                  id: item.shopifyId,
                  tags: tags
                }
              });
              addLog(`Added "${SUPPLIER_TAG}" tag to product "${item.title}"`, 'info');
            }
          }
