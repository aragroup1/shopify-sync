...
function normalizeForMatching(text = '') { return String(text).toLowerCase().replace(/KATEX_INLINE_OPEN[^)]*KATEX_INLINE_CLOSE/g, ' ').replace(/```math
.*?```/g, ' ').replace(/```math.*?```/gs, ' ').replace(/-parcel-large-letter-rate$/i, '').replace(/-p\d+$/i, '').replace(/\b(a|an|the|of|in|on|at|to|for|with|by)\b/g, '').replace(/[^a-z0-9]+/g, ' ').replace(/\s+/g, ' ').trim(); }
