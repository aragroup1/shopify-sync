    ...
    try {
        ...
        for (const apifyProd of apifyProcessed) {
            ...
            if (shopifyProd) {
                ...
                addLog(`[Inventory] SKU Not Found: "${apifyProd.sku}" (Title: "${apifyProd.title}")`, 'warning');
                notFound++;
            }
        }
        ...
        if (notFound > 0) {
            addLog(`Actionable Info: ${notFound} supplier SKUs were not found. To fix, run the 'New Product Sync' to create them, or manually check Shopify for products with missing/mismatched SKUs or tags.`, 'info');
        }
