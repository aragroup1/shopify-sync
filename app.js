...
// In-memory storage - Preserved from your script
    ...
    try {
        ...
        for (const product of allShopifyProducts) {
            ...
            if (variant.inventory_management !== 'shopify') {
                ...
                } catch (e) { addLog(`Error enabling tracking for "${product.title}": ${e.message}`, 'error'); errors++; continue; }
            }
            if (variant.inventory_item_id) {
                ...
                } catch (connectError) { if (connectError.response?.status !== 422) { addLog(`Error connecting "${product.title}": ${connectError.message}`, 'error'); errors++; } }
            }
            if (needsFix) { fixedCount++; await new Promise(r => setTimeout(r, 500)); }
        }
    } catch (e) { addLog(`Critical error during fix job: ${e.message}`, 'error'); errors++; }
...
async function executeInventoryUpdates(updates, token) {
  ...
  for (const update of updates) {
      ...
      } catch (error) { errors++; stats.errors++; addLog(`✗ Failed to set quantity for ${update.title}: ${error.message}`, 'error'); }
      await new Promise(r => setTimeout(r, 500));
  }
