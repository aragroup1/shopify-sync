...
// NEW: Pending discontinue tracking for two-phase verification
...
// NEW: Core SKUs that must always be present
...
// NEW: Generate checksum for data integrity validation
...
// NEW: Validate Apify data freshness
async function validateApifyFreshness() {
    try {
        const { data } = await apifyClient.get(
            `/acts/${config.apify.actorId}/runs/last?token=${config.apify.token}`
        );
        
        if (!data.finishedAt) {
            throw new Error('Apify run has not finished yet or data is unavailable');
        }
        ...
        if (hoursSinceUpdate > FETCH_VALIDATION.MAX_DATA_AGE_HOURS) {
            throw new Error(
                `Apify data is ${hoursSinceUpdate.toFixed(1)} hours old ` +
                `(max allowed: ${FETCH_VALIDATION.MAX_DATA_AGE_HOURS} hours). ` +
                `The data is too stale for safe synchronization.`
            );
        }
        
        addLog(`✓ Apify data freshness validated (${hoursSinceUpdate.toFixed(1)} hours old)`, 'success');
        return { finishedAt, hoursSinceUpdate };
    } catch (error) {
        if (error.message.includes('hours old')) throw error;
        throw new Error(`Failed to validate Apify data freshness: ${error.message}`);
    }
}
...
function validateFetchCompleteness(dataType, count, additionalData = {}, checksum = null) {
    ...
    // Store fetch history with checksum
    if (dataType === 'apify') {
    ...
    // Check for sudden drops compared to recent history
...
async function performReconciliationCheck(apifyData, shopifyData) {
    ...
    const matchPercentage = (matchedCount / apifySkuSet.size) * 100;
    
    // Check for orphaned Shopify products
...
// Enhanced Data Fetching Functions with all validations
async function getApifyProducts() {
    ...
    try {
        ...
        addLog(`Apify freshness check failed: ${freshnessError.message}`, 'error');
    ...
    for (let attempt = 1; attempt <= FETCH_VALIDATION.FETCH_RETRY_ATTEMPTS; attempt++) {
        try {
            ...
            while (true) {
                const { data } = await apifyClient.get(
                    `/acts/${config.apify.actorId}/runs/last/dataset/items?token=${config.apify.token}&limit=1000&offset=${offset}`
                );
...
async function getShopifyProducts({ fields = 'id,handle,title,variants,tags,status,created_at' } = {}) {
    ...
    for (let attempt = 1; attempt <= FETCH_VALIDATION.FETCH_RETRY_ATTEMPTS; attempt++) {
        try {
            ...
            // Generate checksum for supplier products
...
// --- CORE JOB LOGIC (Enhanced with all validations) ---
...
// NEW: Two-phase discontinued products handling
async function handleDiscontinuedProductsJob(token, { overrideFailsafe = false } = {}) {
    ...
    try {
        ...
        // Process two-phase discontinuation
        for (const product of potentialDiscontinued) {
            ...
            if (pendingDiscontinue.has(pendingKey)) {
                ...
                if (timeSincePending >= pendingThreshold) {
                    // Been pending long enough, actually discontinue
                ...
                // First time seeing this as missing, mark as pending
        ...
        // Additional validation before discontinuing
...
app.post('/api/update-baseline', (req, res) => {
    ...
    // Update the validation thresholds
