...
function cleanProductTitle(title) {
  ...
  // First remove anything in parentheses (like parcel rates)
  let cleaned = title.replace(/\s*KATEX_INLINE_OPEN[^)]*KATEX_INLINE_CLOSE\s*/g, ' ');
  
  // Then remove SKU-like codes (4+ digits, optionally preceded by letters)
  cleaned = cleaned.replace(/\b[A-Z]{0,2}\d{4,}\b/gi, '');
