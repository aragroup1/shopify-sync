...
// --- CORRECTED FUNCTION BASED ON CSV STRUCTURE ---
function processApifyProducts(apifyData, { processPrice = true } = {}) {
    return apifyData.map(item => {
        ...
        // Inventory logic - check stock status
        let inventory = 20; // default
        if (item.variants?.[0]?.price?.stockStatus === 'OutOfStock') {
            inventory = 0;
        }
        ...
        // Price from variants/0/price/current (in pence, so divide by 100)
        let price = '0.00';
