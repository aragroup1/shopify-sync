...
// Pending discontinue tracking for two-phase verification
...
// Enhanced fetch validation configuration
...
// Core SKUs that must always be present
...
function startBackgroundJob(key, name, fn) {
    ...
    if (failsafeTriggered) {
        addLog(`System in failsafe mode. Cannot start job: ${name}`, 'warning');
        return false;
    }
    jobLocks[key] = true;
...
const TITLE_CLEANUP_PATTERNS = [
    /\s*KATEX_INLINE_OPEN[^)]*KATEX_INLINE_CLOSE\s*/g,
    /\b[A-Z]{1,3}-?\d{4,}[A-Z]?\b/gi,
    /\b\d{4,}\b/g
];
...
function validateSkuContinuity(apifyData) {
    ...
    if (missingCore.length > 0) {
        throw new Error(`Critical SKUs missing from Apify data: ${missingCore.join(', ')}. This indicates a serious data issue.`);
    }
...
// Ensure Apify data is ready - now looks for last SUCCEEDED run
async function ensureApifyDataIsReady(retries = 4, delay = 300000) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            // FIX: We now specifically request the last SUCCEEDED run
            addLog(`Checking for the last SUCCEEDED Apify run (Attempt ${attempt}/${retries})...`, 'info');
            const { data } = await apifyClient.get(
                `/acts/${config.apify.actorId}/runs/last?status=SUCCEEDED&token=${config.apify.token}`
            );

            if (!data || !data.finishedAt) {
                throw new Error('No SUCCEEDED Apify run could be found.');
            }
            ...
            if (hoursSinceUpdate > FETCH_VALIDATION.MAX_DATA_AGE_HOURS) {
                throw new Error(
                    `The last successful Apify run is ${hoursSinceUpdate.toFixed(1)} hours old ` +
                    `(max allowed: ${FETCH_VALIDATION.MAX_DATA_AGE_HOURS} hours). Too stale.`
                );
            }

            addLog(`✓ Found last successful Apify run. Data is fresh (${hoursSinceUpdate.toFixed(1)} hours old)`, 'success');
            return true;

        } catch (error) {
            if (attempt >= retries) {
                throw new Error(`Could not find a valid, recent, and successful Apify run after ${retries} attempts. Last error: ${error.message}`);
            }
            addLog(`Error checking for successful Apify run: ${error.message}. Retrying in ${delay / 60000} minutes...`, 'warning');
            await new Promise(res => setTimeout(res, delay));
        }
    }
}

// Enhanced fetch validation function with checksum
function validateFetchCompleteness(dataType, count, additionalData = {}, checksum = null) {
    ...
    if (dataType === 'apify') {
        ...
        if (checksum) {
            dataChecksums.apify.push({ timestamp, checksum, count });
            if (dataChecksums.apify.length > 5) dataChecksums.apify.shift();
        }

        if (count < FETCH_VALIDATION.MIN_APIFY_PRODUCTS) {
            throw new Error(`Apify fetch returned only ${count} products (minimum expected: ${FETCH_VALIDATION.MIN_APIFY_PRODUCTS})`);
        }
        if (count > FETCH_VALIDATION.MAX_APIFY_PRODUCTS) {
            throw new Error(`Apify fetch returned ${count} products (maximum expected: ${FETCH_VALIDATION.MAX_APIFY_PRODUCTS})`);
        }
        ...
        if (checksum) {
            dataChecksums.shopify.push({ timestamp, checksum, count });
            if (dataChecksums.shopify.length > 5) dataChecksums.shopify.shift();
        }

        if (count < FETCH_VALIDATION.MIN_SHOPIFY_SUPPLIER_PRODUCTS) {
            throw new Error(`Shopify fetch returned only ${count} supplier products (minimum expected: ${FETCH_VALIDATION.MIN_SHOPIFY_SUPPLIER_PRODUCTS})`);
        }
    }
    ...
    if (recentHistory.length >= 3) {
        ...
        if (dropPercentage > 10) {
            throw new Error(`${dataType} fetch shows ${dropPercentage.toFixed(1)}% drop from recent average (${Math.round(recentAvg)} -> ${count})`);
        }
    }
    ...
    if (checksum && dataChecksums[dataType === 'apify' ? 'apify' : 'shopify'].length >= 2) {
        ...
        if (lastChecksum.count === count && lastChecksum.checksum !== checksum) {
            addLog(`⚠️ Warning: Same product count but different checksum detected for ${dataType}. Data content may have changed.`, 'warning');
        }
    }
...
// Enhanced reconciliation check function
async function performReconciliationCheck(apifyData, shopifyData) {
    ...
    for (const product of supplierProducts) {
        ...
        if (sku) shopifySkuSet.add(sku);
    }

    let matchedCount = 0;
    let unmatchedApifySkus = [];

    for (const apifySku of apifySkuSet) {
        if (shopifySkuSet.has(apifySku)) {
            matchedCount++;
        } else {
            unmatchedApifySkus.push(apifySku);
        }
    }
    ...
    for (const shopifySku of shopifySkuSet) {
        if (!apifySkuSet.has(shopifySku)) {
            orphanedShopifySkus.push(shopifySku);
        }
    }
    ...
    if (unmatchedApifySkus.length > 0 && unmatchedApifySkus.length <= 10) {
        addLog(`Sample unmatched Apify SKUs: ${unmatchedApifySkus.slice(0, 10).join(', ')}`, 'warning');
    }

    if (orphanedShopifySkus.length > 0 && orphanedShopifySkus.length <= 10) {
        addLog(`Sample orphaned Shopify SKUs: ${orphanedShopifySkus.slice(0, 10).join(', ')}`, 'warning');
    }

    return {
        apifyCount: apifySkuSet.size,
        shopifyCount: supplierProducts.length,
        matchedCount,
        matchPercentage,
        unmatchedCount: unmatchedApifySkus.length,
        orphanedCount: orphanedShopifySkus.length,
        isValid: matchPercentage >= FETCH_VALIDATION.REQUIRED_MATCH_PERCENTAGE,
        unmatchedApifySkus,
        orphanedShopifySkus
    };
}
...
// Get Apify products - now fetches from last SUCCEEDED run
async function getApifyProducts() {
    let lastError = null;

    // First, ensure a valid run exists
    try {
        await ensureApifyDataIsReady();
    ...
    for (let attempt = 1; attempt <= FETCH_VALIDATION.FETCH_RETRY_ATTEMPTS; attempt++) {
        try {
            addLog(`Fetching Apify product items from last SUCCEEDED run (attempt ${attempt}/${FETCH_VALIDATION.FETCH_RETRY_ATTEMPTS})...`, 'info');
            ...
            while (true) {
                // FIX: We fetch items from the last SUCCEEDED run
                const { data } = await apifyClient.get(
                    `/acts/${config.apify.actorId}/runs/last/dataset/items?status=SUCCEEDED&token=${config.apify.token}&limit=1000&offset=${offset}`
                );
                ...
                if (!data || data.length === 0) {
                    ...
                    const retryResponse = await apifyClient.get(
                        `/acts/${config.apify.actorId}/runs/last/dataset/items?status=SUCCEEDED&token=${config.apify.token}&limit=1000&offset=${offset}`
                    );
                    if (!retryResponse.data || retryResponse.data.length === 0) break;
                    allItems.push(...retryResponse.data);
                } else {
                ...
                if (offset % 3000 === 0) {
                    addLog(`Apify fetch progress: ${allItems.length} products fetched so far...`, 'info');
                }
            }
    ...
    for (let attempt = 1; attempt <= FETCH_VALIDATION.FETCH_RETRY_ATTEMPTS; attempt++) {
        try {
            ...
            while (url) {
                ...
                if (pageCount % 10 === 0) {
                    addLog(`Shopify fetch progress: ${allProducts.length} products fetched (${pageCount} pages)...`, 'info');
                }
            ...
            validateFetchCompleteness('shopify-supplier', supplierProducts.length, {
...
function normalizeForMatching(text = '') {
    return String(text).toLowerCase()
        ...
        .replace(/```math.*?```/gs, ' ')
        .replace(/-parcel-large-letter-rate$/i, '')
        .replace(/-p\d+$/i, '')
...
// --- CORE JOB LOGIC ---
// (The logic functions: deduplicateProductsJob, improvedMapSkusJob, updateInventoryJob, createNewProductsJob, handleDiscontinuedProductsJob, cleanseUnmatchedProductsJob, generateAndSendErrorReport are unchanged from the previous correct version and can be copied from there if needed)
...
async function improvedMapSkusJob(token) {
    ...
    let matchedByPartialSkuInSku = 0, matchedByPartialSkuInHandle = 0, matchedByPartialSkuInTitle = 0;
    ...
    try {
        ...
        const reconciliation = await performReconciliationCheck(apifyData, shopifyData);
        if (!reconciliation.isValid) {
            addLog(`SKU mapping aborted: Reconciliation check failed (${reconciliation.matchPercentage.toFixed(1)}% match)`, 'error');
            return;
        }
        ...
        for (const shopifyProd of toProcess) {
            ...
            let bestMatch = null, matchType = '';

            for (const apifyProd of remainingApifyProducts) {
                if (shopifyProd.handle.toLowerCase() === apifyProd.handle.toLowerCase()) {
                    bestMatch = apifyProd;
                    matchType = 'handle';
                    break;
                }
            ...
            if (bestMatch) {
                ...
                if (bestMatch) {
                    ...
                    if (shopifySku && /^\d+$/.test(shopifySku) && shopifySku.length >= 4) {
                        for (const apifyProd of remainingApifyProducts) {
                            if (apifyProd.sku.includes(shopifySku)) {
                                bestMatch = apifyProd;
                                matchType = 'partial-sku-in-full-sku';
                                matchedByPartialSkuInSku++;
                                break;
                            }
                            if (apifyProd.handle.includes(shopifySku)) {
                                bestMatch = apifyProd;
                                matchType = 'partial-sku-in-handle';
                                matchedByPartialSkuInHandle++;
                                break;
                            }
                            if (apifyProd.title.includes(shopifySku)) {
                                bestMatch = apifyProd;
                                matchType = 'partial-sku-in-title';
                                matchedByPartialSkuInTitle++;
                                break;
                            }
                        }
                    }
        ...
        addLog(`- Matched by Partial SKU (in SKU/Handle/Title): ${matchedByPartialSkuInSku}/${matchedByPartialSkuInHandle}/${matchedByPartialSkuInTitle}`, 'info');
        addLog(`- Matched by fuzzy: ${matchedByFuzzy}`, 'info');
        ...
        if (updateCandidates.length > 0) {
            ...
            for (const item of updateCandidates) {
                if (shouldAbort(token)) break;
                try {
                    await shopifyRequestWithRetry(shopifyClient.put, `/variants/${item.variantId}.json`, {
                        variant: { id: item.variantId, sku: item.newSku }
                    });
                    updated++;
    ...
    lastRun.mapSkus = {
        ...
        updated, errors, alreadyMatched, noMatch,
        matchedByHandle, matchedByTitle,
        matchedByPartialSkuInSku, matchedByPartialSkuInHandle, matchedByPartialSkuInTitle,
        matchedByFuzzy, skippedNonSupplier
    };
}

async function updateInventoryJob(token, { overrideFailsafe = false } = {}) {
    addLog('Starting inventory sync with comprehensive validation...', 'info');
    ...
    try {
        ...
        const reconciliation = await performReconciliationCheck(apifyData, shopifyData);
        if (!reconciliation.isValid && !overrideFailsafe) {
            throw new Error(
                `Reconciliation check failed: Only ${reconciliation.matchPercentage.toFixed(1)}% of Apify products found in Shopify. ` +
                `This indicates a potential data issue. Aborting inventory update to prevent errors.`
            );
        }
        ...
        for (const apifyProd of apifyProcessed) {
            ...
            if (matchResult.product) {
                ...
                notFound++;
                if (notFoundItems.length < 100) notFoundItems.push({
        ...
        if (updates.length > 0) {
            ...
            for (const update of updates) {
                if (shouldAbort(token)) break;
                try {
                    await shopifyRequestWithRetry(shopifyClient.post, '/inventory_levels/set.json', update);
        ...
        if (statusUpdates.length > 0) {
            for (const update of statusUpdates) {
                if (shouldAbort(token)) break;
                try {
                    await shopifyRequestWithRetry(shopifyClient.put, `/products/${update.id}.json`, {
                        product: { id: update.id, status: update.newStatus }
                    });
        ...
        if (e.message.includes('exceeds failsafe limit') || e.message.includes('Reconciliation')) {
...
async function createNewProductsJob(token, { overrideFailsafe = false } = {}) {
    ...
    try {
        ...
        const reconciliation = await performReconciliationCheck(apifyData, shopifyData);
        addLog(`Reconciliation: ${reconciliation.matchPercentage.toFixed(1)}% match, ${reconciliation.unmatchedCount} unmatched Apify products`, 'info');
...
// Two-phase discontinued products handling
async function handleDiscontinuedProductsJob(token, { overrideFailsafe = false } = {}) {
    addLog('Starting TWO-PHASE discontinued check with comprehensive validation...', 'info');
    let discontinued = 0, errors = 0, skipped = 0, markedPending = 0;
    ...
    try {
        ...
        if (reconciliation.unmatchedCount > 100 && !overrideFailsafe) {
            throw new Error(
                `Found ${reconciliation.unmatchedCount} unmatched Apify products. ` +
                `This is unusually high and may indicate a fetch issue. ` +
                `Aborting discontinuation to prevent false positives.`
            );
        }
        ...
        addLog(`Checking ${activeSupplierProducts.length} active products against Apify SKUs...`, 'info');

        const now = Date.now();
        const pendingThreshold = FETCH_VALIDATION.PENDING_DISCONTINUE_HOURS * 60 * 60 * 1000;
        ...
        for (const product of potentialDiscontinued) {
            const sku = product.variants?.[0]?.sku;
            if (!sku) continue;
            const pendingKey = `${product.id}_${sku}`;

            if (pendingDiscontinue.has(pendingKey)) {
                const pendingInfo = pendingDiscontinue.get(pendingKey);
                const timeSincePending = now - pendingInfo.timestamp;

                if (timeSincePending >= pendingThreshold) {
                    discontinuedItems.push(product);
                    pendingDiscontinue.delete(pendingKey);
                } else {
                    const hoursRemaining = ((pendingThreshold - timeSincePending) / (60 * 60 * 1000)).toFixed(1);
                    addLog(`Product "${product.title}" (SKU: ${sku}) still pending discontinuation (${hoursRemaining} hours remaining)`, 'warning');
                    pendingItems.push(product);
                }
            } else {
                pendingDiscontinue.set(pendingKey, {
                    timestamp: now,
                ...
                markedPending++;
                pendingItems.push(product);
                addLog(`Marked "${product.title}" (SKU: ${sku}) as PENDING discontinuation (will be discontinued in ${FETCH_VALIDATION.PENDING_DISCONTINUE_HOURS} hours if still missing)`, 'warning');
            }
        }

        addLog(`Discontinuation summary: ${discontinuedItems.length} to discontinue now, ${pendingItems.length} pending, ${markedPending} newly marked as pending`, 'info');

        if (discontinuedItems.length > 50 && !overrideFailsafe) {
            const samples = discontinuedItems.slice(0, 5).map(p =>
                `"${p.title}" (SKU: ${p.variants?.[0]?.sku})`
            ).join(', ');

            throw new Error(
                `About to discontinue ${discontinuedItems.length} products. ` +
                `This seems unusually high. Samples: ${samples}. ` +
                `Please verify the Apify data is complete.`
            );
        }

        if (discontinuedItems.length > 0) {
            const discontinuePercentage = supplierProducts.length > 0 ? (discontinuedItems.length / supplierProducts.length) * 100 : 0;
            ...
            if (overrideFailsafe) addLog(`OVERRIDE: Proceeding with discontinuing ${discontinuedItems.length} products.`, 'warning');

            for (const product of discontinuedItems) {
                if (shouldAbort(token)) break;
                try {
                    ...
                    await shopifyRequestWithRetry(shopifyClient.put, `/products/${product.id}.json`, {
            ...
            skipped = discontinuedItems.length - discontinued;
        } else if (pendingItems.length > 0) {
            addLog(`${pendingItems.length} products are pending discontinuation. They will be discontinued if still missing in the next check.`, 'warning');
        } else {
            addLog('No products to discontinue.', 'success');
        }

        const oldThreshold = now - (pendingThreshold * 2);
        for (const [key, info] of pendingDiscontinue.entries()) {
            if (info.timestamp < oldThreshold) {
                pendingDiscontinue.delete(key);
                addLog(`Cleared old pending entry for product ID ${info.productId}`, 'info');
            }
        }

        stats.discontinued += discontinued;
    } catch (e) {
        if (e.message.includes('exceeds failsafe limit') || e.message.includes('fetch') || e.message.includes('validation') || e.message.includes('reconciliation')) {
            failsafeTriggered = true;
            failsafeReason = e.message;
            ...
            const msg = `<b>APIFY SYNC: FAILSAFE TRIGGERED</b>\n${e.message}\nNo products were discontinued. Please check the data sources.`;
            addLog(e.message, 'error');
            await notifyTelegram(msg);
            return;
        }
...
async function cleanseUnmatchedProductsJob(token, { overrideFailsafe = false } = {}) {
    addLog('--- Starting Unmatched Product Cleanup Job with Full Validation ---', 'error');
    ...
    try {
        ...
        if (!reconciliation.isValid && !overrideFailsafe) {
            throw new Error(
                `Reconciliation check failed (${reconciliation.matchPercentage.toFixed(1)}% match). ` +
                `Cannot safely perform deletion when data sources are not in sync.`
            );
        }
        ...
        if (overrideFailsafe) addLog(`OVERRIDE: Proceeding with cleanse despite validation warnings.`, 'warning');

        const apifyProcessed = processApifyProducts(apifyData, { processPrice: false });
        const apifySkuMap = new Map(apifyProcessed.map(p => [p.sku.toLowerCase(), p]));
        const supplierProducts = shopifyData.filter(p => p.tags && p.tags.includes(SUPPLIER_TAG));

        addLog(`Found ${supplierProducts.length} Shopify products with tag '${SUPPLIER_TAG}' to check.`, 'info');
        ...
        for (const shopifyProd of supplierProducts) {
            ...
            const shopifySku = variant.sku?.toLowerCase();
            if (shopifySku && apifySkuMap.has(shopifySku)) {
                matchedShopifyIds.add(shopifyProd.id);
                continue;
            }
            const partialSku = variant.sku;
            if (partialSku && /^\d+$/.test(partialSku) && partialSku.length >= 4) {
                const partialMatch = apifyProcessed.find(p =>
                    p.sku.includes(partialSku) ||
                    p.handle.includes(partialSku) ||
                    p.title.includes(partialSku)
                );
                if (partialMatch) {
                    matchedShopifyIds.add(shopifyProd.id);
                    continue;
            ...
            const normTitle = normalizeForMatching(shopifyProd.title);
            const fuzzyMatch = apifyProcessed.find(p => getWordOverlap(normTitle, p.normalizedTitle) > 65);
            if (fuzzyMatch) {
                matchedShopifyIds.add(shopifyProd.id);
                continue;
            }
        ...
        const toDelete = supplierProducts.filter(p => !matchedShopifyIds.has(p.id));
        toDeleteCount = toDelete.length;

        addLog(`Found ${matchedShopifyIds.size} matched products.`, 'info');
        addLog(`${toDeleteCount} products have no match and will be deleted.`, 'warning');

        if (toDeleteCount > 0) {
            addLog(`Examples of products to be deleted:`, 'warning');
            toDelete.slice(0, 5).forEach(p =>
                addLog(`  - "${p.title}" (SKU: ${p.variants?.[0]?.sku || '(none)'})`, 'warning')
            );
            addLog('Deletion will begin in 5 seconds...', 'error');
            await new Promise(r => setTimeout(r, 5000));

            for (const product of toDelete) {
                if (shouldAbort(token)) break;
                try {
                    await shopifyRequestWithRetry(shopifyClient.delete, `/products/${product.id}.json`);
                    deletedCount++;
                    addLog(` ✓ Deleted product ID ${product.id} ("${product.title}")`, 'success');
                } catch (e) {
                    errors++;
                    addLog(` ✗ Error deleting product ID ${product.id}: ${e.message}`, 'error', e);
                }
            }
        } else {
            addLog('No unmatched products found to delete.', 'success');
        }
    } catch (e) {
        if (e.message.includes('too low to safely perform a deletion') || e.message.includes('Reconciliation')) {
            failsafeTriggered = true;
            failsafeReason = e.message;
...
async function generateAndSendErrorReport() {
    try {
        ...
        const sortedErrors = [...errorSummary.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10);
...
// --- UI AND API ---
// REFACTORED: Logic moved outside of res.send for stability
app.get('/', (req, res) => {
    const status = systemPaused ? 'PAUSED' : (failsafeTriggered ? 'FAILSAFE' : 'RUNNING');
    const pendingCount = pendingDiscontinue.size;

    const logsHtml = logs.map(log => {
        const logClass = log.type === 'error' ? 'log-error' : log.type === 'warning' ? 'log-warning' : log.type === 'success' ? 'log-success' : '';
        return `<div class="log-entry ${logClass}"><small>${new Date(log.timestamp).toLocaleString()}</small><span>${log.message}</span></div>`;
    }).join('');
    ...
    const systemButtonsHtml = systemPaused ?
        `<button class="btn btn-success" onclick="fetch('/api/pause',{method:'POST',body:JSON.stringify({paused:!1}),headers:{'Content-Type':'application/json'}}).then(()=>location.reload())"><i class="fas fa-play"></i> Resume System</button>` :
        `<button class="btn btn-warning" onclick="fetch('/api/pause',{method:'POST',body:JSON.stringify({paused:!0}),headers:{'Content-Type':'application/json'}}).then(()=>location.reload())"><i class="fas fa-pause"></i> Pause System</button>`;

    const failsafeHtml = failsafeTriggered ?
    ...
    <head>
        ...
        <style>
            :root{--primary-color:#4361ee;--secondary-color:#3a0ca3;--success-color:#4cc9f0;--warning-color:#f72585;--info-color:#4895ef;--light-color:#f8f9fa;--dark-color:#212529}
            body{font-family:'Inter',-apple-system,BlinkMacSystemFont,sans-serif;background-color:#f7f7f9;color:#333;line-height:1.6;padding:0;margin:0}
            .navbar{background:linear-gradient(90deg,var(--primary-color),var(--secondary-color));box-shadow:0 4px 12px rgba(0,0,0,.1);padding:1rem 2rem;color:white;margin-bottom:2rem}
            .navbar h1{margin:0;font-weight:700;font-size:1.5rem}
            .card{border:none;border-radius:12px;box-shadow:0 5px 15px rgba(0,0,0,.05);transition:transform .3s,box-shadow .3s;margin-bottom:25px;overflow:hidden}
            .card:hover{transform:translateY(-5px);box-shadow:0 10px 20px rgba(0,0,0,.1)}
            .card-header{background:white;padding:1.25rem 1.5rem;font-weight:600;border-bottom:1px solid rgba(0,0,0,.05);display:flex;align-items:center;justify-content:space-between}
            .card-body{padding:1.5rem}
            .status-badge{font-size:.9rem;padding:.5rem 1rem;border-radius:50px;font-weight:500;text-transform:uppercase;letter-spacing:.5px;box-shadow:0 3px 8px rgba(0,0,0,.1)}
            .btn{border-radius:50px;padding:.6rem 1.5rem;font-weight:500;text-transform:capitalize;letter-spacing:.3px;transition:all .3s;box-shadow:0 3px 6px rgba(0,0,0,.1);margin:.25rem}
            .btn:hover{transform:translateY(-2px);box-shadow:0 5px 10px rgba(0,0,0,.15)}
            .btn i{margin-right:5px}
            .btn-primary{background:linear-gradient(45deg,var(--primary-color),var(--info-color));border:none}
            .btn-warning{background:linear-gradient(45deg,#f72585,#ff9e00);border:none;color:white}
            .btn-success{background:linear-gradient(45deg,#06d6a0,#1b9aaa);border:none}
            .btn-danger{background:linear-gradient(45deg,#ef476f,#ffd166);border:none}
            .btn-secondary{background:linear-gradient(45deg,#8338ec,#3a86ff);border:none;color:white}
            .stat-card{padding:1rem;border-radius:10px;box-shadow:0 3px 10px rgba(0,0,0,.05);background:white;text-align:center;transition:all .3s}
            .stat-card:hover{transform:translateY(-3px);box-shadow:0 5px 15px rgba(0,0,0,.1)}
            .stat-card h3{font-size:2rem;font-weight:700;margin:.5rem 0;background:linear-gradient(45deg,var(--primary-color),var(--info-color));-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent}
            .stat-card p{margin:0;font-size:.9rem;color:#6c757d;font-weight:500}
            .logs-container{max-height:500px;overflow-y:auto;border-radius:8px;background:#f8f9fa;padding:1rem}
            .log-entry{padding:.75rem 1rem;margin-bottom:.5rem;border-radius:6px;background:white;box-shadow:0 2px 5px rgba(0,0,0,.02);border-left:4px solid #dee2e6;font-size:.9rem}
            .log-entry small{display:block;font-size:.75rem;opacity:.7;margin-bottom:.25rem}
            .log-error{border-left-color:#ef476f;background:#fff5f7}
            .log-warning{border-left-color:#ffd166;background:#fff9eb}
            .log-success{border-left-color:#06d6a0;background:#f0fff4}
            .alert{border-radius:10px;padding:1rem 1.5rem;border:none;box-shadow:0 3px 10px rgba(0,0,0,.05)}
            .bg-running{background:linear-gradient(-45deg,#06d6a0,#1b9aaa,#4cc9f0,#3a86ff);background-size:400% 400%;animation:gradient 3s ease infinite}
            .bg-paused{background:linear-gradient(-45deg,#ffd166,#ffbd00,#ff9e00,#ff7700);background-size:400% 400%;animation:gradient 3s ease infinite}
            .bg-failsafe{background:linear-gradient(-45deg,#ef476f,#f72585,#b5179e,#7209b7);background-size:400% 400%;animation:gradient 3s ease infinite}
            @keyframes gradient{0%{background-position:0 50%}50%{background-position:100% 50%}100%{background-position:0 50%}}
            .fetch-health{background:#f8f9fa;padding:1rem;border-radius:8px;font-size:.85rem}
            .pending-badge{background:linear-gradient(45deg,#ffd166,#ff9e00);color:white;padding:.25rem .75rem;border-radius:20px;font-size:.8rem;font-weight:600}
        </style>
    </head>
    <body>
        <nav class="navbar"><div class="container-fluid"><h1><i class="fas fa-sync-alt me-2"></i> Shopify Sync Dashboard</h1></div></nav>
        <div class="container">
            <div class="row mb-4">
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header"><h5 class="mb-0"><i class="fas fa-tachometer-alt me-2"></i>System Status</h5>${statusBadgeHtml}</div>
                        <div class="card-body">
                            <div class="row mb-4">
                                <div class="col-6 col-md-3"><div class="stat-card"><p>New Products</p><h3>${stats.newProducts}</h3></div></div>
                                <div class="col-6 col-md-3"><div class="stat-card"><p>Inventory Updates</p><h3>${stats.inventoryUpdates}</h3></div></div>
                                <div class="col-6 col-md-3"><div class="stat-card"><p>Discontinued</p><h3>${stats.discontinued}</h3></div></div>
                                <div class="col-6 col-md-3"><div class="stat-card"><p>Errors</p><h3>${stats.errors}</h3></div></div>
                            </div>
                            ${pendingHtml}
                            <div class="fetch-health">
                                <strong>Validation Settings:</strong><br>
                                Apify: ${FETCH_VALIDATION.MIN_APIFY_PRODUCTS}-${FETCH_VALIDATION.MAX_APIFY_PRODUCTS} products<br>
                                Shopify Supplier: Min ${FETCH_VALIDATION.MIN_SHOPIFY_SUPPLIER_PRODUCTS} products<br>
                                Required Match: ${FETCH_VALIDATION.REQUIRED_MATCH_PERCENTAGE}%<br>
                                Data Age Limit: ${FETCH_VALIDATION.MAX_DATA_AGE_HOURS} hours<br>
                                Discontinue Delay: ${FETCH_VALIDATION.PENDING_DISCONTINUE_HOURS} hours<br>
                                ${coreSkusHtml}
                            </div>
                            <div class="d-flex flex-wrap justify-content-center gap-2 mt-3">
                                ${systemButtonsHtml}
                                <button class="btn btn-secondary" onclick="showBaselineModal()"><i class="fas fa-cog"></i> Update Baselines</button>
                            </div>
                            ${failsafeHtml}
                        </div>
                    </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="d-flex flex-wrap justify-content-center gap-2">
                                <button class="btn btn-warning" onclick="runSync('improved-map-skus')" ${failsafeTriggered ? 'disabled' : ''}><i class="fas fa-map-signs"></i> Comprehensive SKU Mapping</button>
                                <button class="btn btn-primary" onclick="runSync('inventory')" ${failsafeTriggered ? 'disabled' : ''}><i class="fas fa-boxes"></i> Sync Inventory</button>
                                <button class="btn btn-primary" onclick="runSync('products')" ${failsafeTriggered ? 'disabled' : ''}><i class="fas fa-plus-circle"></i> Sync New Products</button>
                                <button class="btn btn-primary" onclick="runSync('discontinued')" ${failsafeTriggered ? 'disabled' : ''}><i class="fas fa-archive"></i> Check Discontinued</button>
                                <button class="btn btn-secondary" onclick="runSync('deduplicate')" ${failsafeTriggered ? 'disabled' : ''}><i class="fas fa-clone"></i> Find & Delete Duplicates</button>
                                <button class="btn btn-danger" onclick="if(confirm('DANGER: This will permanently delete all supplier products that cannot be matched. This cannot be undone. Are you sure you want to proceed?')) runSync('cleanse-unmatched')" ${failsafeTriggered ? 'disabled' : ''}><i class="fas fa-trash-alt"></i> Cleanse Unmatched Products</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header"><h5 class="mb-0"><i class="fas fa-list-alt me-2"></i>Logs</h5></div>
                        <div class="card-body"><div class="logs-container">${logsHtml}</div></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="modal fade" id="baselineModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header"><h5 class="modal-title">Update Baseline Expectations</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                    <div class="modal-body">
                        <form id="baselineForm">
                            <div class="mb-3"><label class="form-label">Min Apify Products</label><input type="number" class="form-control" id="minApify" value="${FETCH_VALIDATION.MIN_APIFY_PRODUCTS}"></div>
                            <div class="mb-3"><label class="form-label">Max Apify Products</label><input type="number" class="form-control" id="maxApify" value="${FETCH_VALIDATION.MAX_APIFY_PRODUCTS}"></div>
                            <div class="mb-3"><label class="form-label">Min Shopify Supplier Products</label><input type="number" class="form-control" id="minShopify" value="${FETCH_VALIDATION.MIN_SHOPIFY_SUPPLIER_PRODUCTS}"></div>
                        </form>
                    </div>
                    <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="button" class="btn btn-primary" onclick="updateBaselines()">Update</button></div>
                </div>
            </div>
        </div>
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js"></script>
        <script>
            function runSync(t){fetch("/api/sync/"+t,{method:"POST"}).then(t=>t.json()).then(t=>{1===t.s?(alert("Job started successfully!"),setTimeout(()=>location.reload(),1e3)):alert(t.msg||"Job already running. Try again later.")}).catch(t=>{alert("Error: "+t)})}
            function showBaselineModal(){new bootstrap.Modal(document.getElementById('baselineModal')).show()}
            function updateBaselines(){const data={minApify:document.getElementById('minApify').value,maxApify:document.getElementById('maxApify').value,minShopify:document.getElementById('minShopify').value};fetch('/api/update-baseline',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(r=>r.json()).then(r=>{if(r.success){alert('Baselines updated successfully!');location.reload()}else{alert('Failed to update baselines: '+(r.error||'Unknown error'))}}).catch(e=>alert('Error: '+e))}
