    ...
    try {
        for (const chunk of chunkArray(inventoryItemIds, 50)) {
            const { data } = await shopifyClient.get(
                `/inventory_levels.json?inventory_item_ids=${chunk.join(',')}&location_ids=${config.shopify.locationId}`
            );
            data.inventory_levels.forEach(level => {
                levels.set(level.inventory_item_id, level.available);
            });
            await new Promise(r => setTimeout(r, 500));
        }
    } catch (error) {
        addLog(`Error fetching inventory levels: ${error.message}`, 'error', error);
        throw error;
    }
    return levels;
}

// Fixed normalizeForMatching function
...
[^```]*```\s*/g, ' ')  // Remove content in brackets
        .replace(/\s*\{[^}]*\}\s*/g, ' ')   // Remove content in braces
