...
// NEW: Failsafe pending state for waiting on user decision
let failsafePending = null; // { reason, context, data, callback, timestamp }
...
// NEW: Wait for failsafe decision with timeout
async function waitForFailsafeDecision(timeoutMs = 300000) { // 5 min timeout
...
// MODIFIED: triggerFailsafe now creates a pending state and waits for decision
async function triggerFailsafe(msg, contextData = {}, allowOverride = true) {
  if (failsafeTriggered) return false;
  
  addLog(`⚠️ APIFY FAILSAFE WARNING: ${msg}`, 'error', 'failsafe');
  
  if (!allowOverride) {
    // Critical failsafe - no override allowed
    ...
    const debugInfo = `
🚨 <b>APIFY CRITICAL Failsafe - System Halted</b> 🚨
...
// MODIFIED: checkFailsafeConditions now returns a promise and can wait for decision
...
// [Helper functions remain the same - normalizeTitle, sanitizeProductTitle, etc.]
// Helpers
...
async function getApifyProducts() {
  ...
  try {
    ...
    stats.errors++;
    await triggerFailsafe(`Apify fetch failed: ${error.message}`, { error: error.message }, false); // Critical - no override
    throw error;
  }
...
async function getShopifyProducts({ onlyApifyTag = true } = {}) {
  ...
  try {
    ...
    stats.errors++;
    await triggerFailsafe(`Shopify fetch failed: ${error.message}`, { error: error.message }, false); // Critical - no override
    throw error;
  }
...
// MODIFIED: Job functions now properly handle async failsafe checks
...
async function updateInventoryJob(token) {
  ...
  try {
    ...
    const canProceed = await checkFailsafeConditions('inventory', { updatesNeeded: inventoryUpdates.length, totalApifyProducts: apifyData.length, errorRate });
    if (!canProceed) {
...
// UI with failsafe pending state
app.get('/', (req, res) => {
  res.send(`
<!DOCTYPE html>
...
<body class="min-h-screen font-sans">
    ...
    <div class="rounded-2xl p-6 card-hover mb-8 fade-in">
      ...
      <div class="mt-4 p-4 rounded-lg bg-gray-700">
        ...
        <ul class="text-xs text-gray-400 mt-2">
          <li>• Now with override option - you can proceed through warnings</li>
          <li>• System will pause and wait for your decision</li>
          <li>• Decisions can be made via dashboard or Telegram</li>
        </ul>
      </div>
    </div>
  ...
  <script>
    ...
    async function fetchAndUpdateStatus() {
      try {
        ...
        if (data.failsafePending !== failsafePending) {
          location.reload(); // Reload to show/hide failsafe pending UI
        }
    ...
    setInterval(fetchAndUpdateStatus, 10000); // Check every 10 seconds for failsafe pending state
  </script>
</body>
</html>
  `);
...
// NEW: Failsafe decision endpoint
...
// MODIFIED: Telegram webhook now handles failsafe decisions
...
// [Sync endpoints remain mostly the same]
...
app.listen(PORT, () => {
  ...
  addLog('Failsafe: Now with override option - system will pause and wait for decision', 'info', 'system');
