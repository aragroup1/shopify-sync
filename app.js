...
// NEW: Debug tracking for inventory updates
const inventoryDebugMap = new Map(); // Track what products keep changing
...
// API clients - FIXED: using axios.create
...
// FIXED: processApifyProducts with proper minimum stock control
...
async function handleDiscontinuedProductsJob(token) {
  ...
  try {
    ...
    // Don't apply minimum stock rule when checking for discontinued
    ...
    // Clear counter for products that are back in stock
...
// NEW: Enhanced inventory update with detailed debugging
async function updateInventoryJob(token) {
  let updated = 0, errors = 0, trackingEnabled = 0, skipped = 0;
  mismatches = [];
  const debugSamples = [];
  const repeatUpdates = [];
  ...
  try {
    ...
    // IMPORTANT: Don't apply minimum stock rule for inventory updates
    ...
    processedProducts.forEach((apifyProduct, idx) => {
      ...
      const rawShopifyQty = shopifyProduct.variants[0].inventory_quantity;
      const rawApifyQty = apifyProduct.inventory;
      
      // Ensure both are numbers for comparison
      const currentInventory = parseInt(rawShopifyQty, 10) || 0;
      const targetInventory = parseInt(rawApifyQty, 10) || 0;
      
      // Debug first 10 products
      if (idx < 10) {
        debugSamples.push({
          title: shopifyProduct.title,
          handle: shopifyProduct.handle,
          shopifyRaw: rawShopifyQty,
          shopifyParsed: currentInventory,
          apifyRaw: rawApifyQty,
          apifyParsed: targetInventory,
          match: currentInventory === targetInventory
        });
      }
      
      if (currentInventory === targetInventory) {
        skipped++;
        // Clear from debug map if it's now in sync
    ...
    if (debugSamples.length > 0) {
      addLog('=== DEBUG: Sample Product Comparisons ===', 'warning', 'inventory');
      debugSamples.forEach(sample => {
        addLog(`${sample.title}: Shopify(raw=${sample.shopifyRaw}, parsed=${sample.shopifyParsed}) vs Apify(raw=${sample.apifyRaw}, parsed=${sample.apifyParsed}) = ${sample.match ? 'MATCH' : 'DIFFERENT'}`, 'warning', 'inventory');
      });
    }
    
    // Check for repeating updates (indicates a bug)
    if (repeatUpdates.length > 0) {
      ...
      repeatUpdates.slice(0, 5).forEach(item => {
        addLog(`REPEAT: ${item.title} - Was set to ${item.previousTarget}, now Shopify shows ${item.currentShopify}, trying to set to ${item.newTarget}`, 'error', 'inventory');
      });
      
      // If too many repeats, there's definitely a bug
    ...
    // Show sample of updates if not too many
    if (inventoryUpdates.length > 0 && inventoryUpdates.length <= 10) {
      addLog(`Sample updates:`, 'info', 'inventory');
      inventoryUpdates.slice(0, 5).forEach(u => {
        addLog(`  - ${u.title}: ${u.currentInventory} → ${u.newInventory}`, 'info', 'inventory');
      });
    }
    ...
    for (const update of inventoryUpdates) {
      ...
      try {
        ...
        await shopifyClient.post('/inventory_levels/set.json', apiPayload);
    ...
    addToHistory('inventory', { updated, errors, attempted: inventoryUpdates.length, trackingEnabled, skipped });
    addLog(`=== INVENTORY UPDATE COMPLETE ===`, 'info', 'inventory');
    addLog(`Result: ${updated} updated, ${skipped} already in sync, ${trackingEnabled} tracking enabled, ${errors} errors`, 'info', 'inventory');
    
    // If we still have many repeating updates, suggest manual investigation
    if (repeatUpdates.length > 50) {
      await notifyTelegram(`⚠️ Inventory Sync Issue Detected!\n\n${repeatUpdates.length} products keep needing updates after being updated.\n\nThis suggests a data format mismatch or API delay.\n\nPlease check the logs for details.`);
    }
          ...
          <tbody id="runHistoryBody">
            ${runHistory.map(r => `
              <tr class="border-b border-gray-700">
                ...
                <td class="px-4 py-2">
                  ${r.type === 'inventory' ? `Updated: ${r.updated || 0}, Skipped: ${r.skipped || 0}, Errors: ${r.errors || 0}` : ''}
...
app.post('/api/sync/products', async (req, res) => {
  const started = startBackgroundJob('products', 'Product sync', async (token) => {
    try {
      ...
      // Apply minimum stock for NEW products
...
app.listen(PORT, () => {
  ...
  addLog('Failsafe: Override enabled - system will pause and wait for decision', 'info', 'system');
  addLog('Enhanced debugging enabled for inventory sync issues', 'info', 'system');
