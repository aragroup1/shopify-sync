...
async function updateInventoryJob(token) {
  ...
  try {
    ...
    for (const apifyProduct of processedProducts) {
      ...
      if (!shopifyProduct.variants?.[0]) continue;
      ...
      if (!variant.inventory_management || variant.inventory_management === 'fulfillment_service') {
        needsTrackingEnabled.push({
          title: shopifyProduct.title,
          handle: shopifyProduct.handle,
          productId: shopifyProduct.id,
          variantId: variant.id,
          inventoryItemId: inventoryItemId,
          targetInventory: apifyProduct.inventory
        });
        
        // Track as problem product
        problemProducts.set(shopifyProduct.handle, {
          title: shopifyProduct.title,
          issue: 'Inventory tracking disabled'
        });
        continue;
      }
      ...
      if (currentInventory === undefined) {
        ...
        problemProducts.set(shopifyProduct.handle, {
          title: shopifyProduct.title,
          issue: 'Not connected to location'
        });
        continue;
      }
      ...
      if (prevUpdate) {
        ...
        // Track as problem product if it keeps repeating
        problemProducts.set(shopifyProduct.handle, {
          title: shopifyProduct.title,
          issue: `Keeps reverting (was ${prevUpdate.targetQty}, now ${currentInventory}, trying ${targetInventory})`
        });
      }
    ...
    if (needsTrackingEnabled.length > 0) {
      addLog(`⚠️ ${needsTrackingEnabled.length} products have inventory tracking DISABLED`, 'warning', 'inventory');
      if (needsTrackingEnabled.length <= 5) {
        needsTrackingEnabled.forEach(p => {
          addLog(`  - ${p.title}: tracking disabled at variant level`, 'warning', 'inventory');
        });
      }
    }
    ...
    if (notConnectedToLocation.length > 0) {
      ...
      if (notConnectedToLocation.length <= 5) {
    ...
    addLog(`- Need tracking enabled: ${needsTrackingEnabled.length}`, 'warning', 'inventory');
    ...
    const totalNeedingWork = inventoryUpdates.length + notConnectedToLocation.length + needsTrackingEnabled.length;
    const errorRate = errors > 0 ? (errors / Math.max(totalNeedingWork, 1) * 100) : 0;
    const canProceed = await checkFailsafeConditions('inventory', { 
      updatesNeeded: totalNeedingWork, 
      totalApifyProducts: apifyData.length, 
      errorRate 
    });
    
    if (!canProceed) {
      return { updated: 0, errors: 0, total: totalNeedingWork };
    }
    
    if (!config.shopify.locationId) {
      addLog('ERROR: SHOPIFY_LOCATION_ID environment variable is not set!', 'error', 'inventory');
      return { updated: 0, errors: 1, total: totalNeedingWork };
    }
    
    // FIRST: Enable tracking for products that need it
    if (needsTrackingEnabled.length > 0) {
      addLog(`Enabling inventory tracking for ${needsTrackingEnabled.length} products...`, 'info', 'inventory');
      for (const product of needsTrackingEnabled) {
        if (shouldAbort(token)) break;
        try {
          await enableInventoryTracking(product.productId, product.variantId);
          trackingFixed++;
          
          // Connect to location after enabling tracking
          await new Promise(r => setTimeout(r, 500));
          ...
          addLog(`✓ Fixed tracking and set inventory for: ${product.title} → ${product.targetInventory}`, 'success', 'inventory');
          updated++;
          stats.inventoryUpdates++;
          problemProducts.delete(product.handle); // Remove from problem list
          
          await new Promise(r => setTimeout(r, 1000));
        } catch (error) {
          errors++;
          stats.errors++;
          addLog(`✗ Failed to fix tracking for ${product.title}: ${error.message}`, 'error', 'inventory');
        }
      }
    }
    
    // SECOND: Connect products not at location
    if (notConnectedToLocation.length > 0) {
    ...
    // THIRD: Update existing inventory levels
    if (inventoryUpdates.length > 0) {
      ...
      for (const update of inventoryUpdates) {
        ...
        try {
          // Double-check tracking is enabled (shouldn't be needed but being safe)
          ...
          updated++;
          stats.inventoryUpdates++;
          problemProducts.delete(update.handle); // Remove from problem list if successful
    ...
    addToHistory('inventory', { 
      updated, 
      errors, 
      attempted: totalNeedingWork, 
      trackingEnabled: trackingEnabled + trackingFixed, 
      skipped,
      connected 
    });
    addLog(`=== INVENTORY UPDATE COMPLETE ===`, 'info', 'inventory');
    addLog(`Result: ${updated} updated, ${connected} connected, ${trackingFixed} tracking fixed, ${skipped} already in sync, ${errors} errors`, 'info', 'inventory');
    
    // If we still have many repeating updates, provide detailed analysis
    if (repeatUpdates.length > 50 || problemProducts.size > 0) {
      let diagMessage = `⚠️ Inventory Sync Analysis\n\n`;
      diagMessage += `${repeatUpdates.length} products still repeating after update.\n`;
      diagMessage += `${needsTrackingEnabled.length} had tracking disabled.\n`;
      diagMessage += `${notConnectedToLocation.length} weren't connected to location.\n`;
      diagMessage += `${connected} products connected this run.\n`;
      diagMessage += `${trackingFixed} products had tracking fixed.\n\n`;
      
      if (problemProducts.size > 0) {
        diagMessage += `Problem products detected:\n`;
        const samples = Array.from(problemProducts.entries()).slice(0, 5);
        samples.forEach(([handle, info]) => {
          diagMessage += `- ${info.title}: ${info.issue}\n`;
        });
      }
      
      diagMessage += `\nPlease run another sync to verify the issue is resolved.`;
      await notifyTelegram(diagMessage);
    }
    
    return { updated, errors, total: totalNeedingWork };
  } catch (error) {
...
<body class="min-h-screen font-sans">
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="rounded-2xl p-6 card-hover mb-8 fade-in">
      ...
      ${problemProducts.size > 0 ? `
      <div class="mt-4 p-4 rounded-lg bg-red-900 border border-red-700">
        <p class="text-sm text-red-400"><strong>⚠️ Problem Products Detected:</strong></p>
        <ul class="text-xs text-red-300 mt-2">
          ${Array.from(problemProducts.entries()).slice(0, 5).map(([handle, info]) => 
            `<li>• ${info.title}: ${info.issue}</li>`
          ).join('')}
          ${problemProducts.size > 5 ? `<li class="text-red-500">... and ${problemProducts.size - 5} more</li>` : ''}
        </ul>
      </div>` : ''}
    </div>
...
app.get('/api/status', (req, res) => {
  res.json({
    ...
    problemProducts: Array.from(problemProducts.entries()).slice(0, 20)
  });
});

// NEW: Diagnostic endpoint for debugging specific products
app.get('/api/debug/inventory/:handle', async (req, res) => {
  try {
    const handle = req.params.handle;
    
    // Get the product
    const productResp = await shopifyClient.get(`/products.json?handle=${handle}`);
    ...
    const inventoryItemResp = await shopifyClient.get(`/inventory_items/${variant.inventory_item_id}.json`);
    const inventoryItem = inventoryItemResp.data.inventory_item;
    
    // Get inventory levels for all locations
    const levelsResp = await shopifyClient.get(`/inventory_levels.json?inventory_item_ids=${variant.inventory_item_id}`);
    const levels = levelsResp.data.inventory_levels;
    
    // Get all locations
    const locationsResp = await shopifyClient.get('/locations.json');
    const locations = locationsResp.data.locations;
    
    // Check if product is in problem list
    const problemInfo = problemProducts.get(handle);
    ...
    res.json({
      ...
      inventory_levels: levels.map(l => ({
        ...
        location_name: locations.find(loc => loc.id === l.location_id)?.name || 'Unknown',
        available: l.available,
        updated_at: l.updated_at
      })),
      configured_location_id: config.shopify.locationId,
      all_locations: locations.map(l => ({
        id: l.id,
        name: l.name,
        active: l.active
      })),
      problem_status: problemInfo || null,
      diagnosis: {
        has_tracking: !!variant.inventory_management && variant.inventory_management !== 'fulfillment_service',
        connected_to_location: levels.some(l => l.location_id === parseInt(config.shopify.locationId)),
        inventory_at_configured_location: levels.find(l => l.location_id === parseInt(config.shopify.locationId))?.available || 'Not connected'
      }
    });
  } catch (error) {
    res.status(500).json({ 
...
app.listen(PORT, () => {
  ...
  addLog('Enhanced diagnostics: Problem product tracking and debug endpoint added', 'info', 'system');
  addLog('Inventory sync now checks and fixes tracking before updating', 'info', 'system');
...
1. **Enhanced diagnostic tracking** - Identifies WHY products keep repeating (tracking disabled, not connected to location, etc.)
2. **Debug endpoint** at `/api/debug/inventory/:handle` to inspect individual products
3. **Automatic fixing** of tracking issues during inventory updates
4. **Problem product tracking** visible in the UI and failsafe messages
5. **Better error handling** for connection and tracking issues

To debug the problematic products, use the debug endpoint:
```
https://your-app-url/api/debug/inventory/cotton-cooking-twine-rope-5-x-4-cm-pack-of-3
```

This will show you exactly what's wrong with each product that keeps repeating.
