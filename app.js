...
// NEW: Fetch validation configuration
...
// NEW: Enhanced fetch validation function
function validateFetchCompleteness(dataType, count, additionalData = {}) {
    const timestamp = new Date().toISOString();
    
    // Store fetch history
    if (dataType === 'apify') {
        fetchHistory.apify.push({ timestamp, count });
        ...
        fetchHistory.shopify.push({ timestamp, count, ...additionalData });
...
// NEW: Reconciliation check function
async function performReconciliationCheck(apifyData, shopifyData) {
    addLog('Performing reconciliation check...', 'warning');
...
async function getApifyProducts() {
    ...
    for (let attempt = 1; attempt <= FETCH_VALIDATION.FETCH_RETRY_ATTEMPTS; attempt++) {
        try {
            ...
            addLog(`✓ Apify fetch validated: ${allItems.length} products (within expected range)`, 'success');
            return allItems;
...
async function getShopifyProducts({ fields = 'id,handle,title,variants,tags,status,created_at' } = {}) {
    ...
    for (let attempt = 1; attempt <= FETCH_VALIDATION.FETCH_RETRY_ATTEMPTS; attempt++) {
        try {
            ...
            addLog(`✓ Shopify fetch validated: ${allProducts.length} total (${supplierProducts.length} supplier, ${activeCount} active, ${draftCount} draft, ${archivedCount} archived)`, 'success');
            return allProducts;
...
// --- CORE JOB LOGIC (Enhanced with validation) ---
...
async function updateInventoryJob(token, { overrideFailsafe = false } = {}) {
    addLog('Starting inventory sync with validation...', 'info');
...
async function handleDiscontinuedProductsJob(token, { overrideFailsafe = false } = {}) {
    addLog('Starting SAFE discontinued check with validation...', 'info');
    ...
    try {
        ...
        addLog(`Found ${potentialDiscontinued.length} active products in Shopify that are no longer in the Apify data.`, 'info');
        
        // Additional validation before discontinuing
        if (potentialDiscontinued.length > 50 && !overrideFailsafe) {
            const samples = potentialDiscontinued.slice(0, 5).map(p => 
                `"${p.title}" (SKU: ${p.variants?.[0]?.sku})`
            ).join(', ');
            
            throw new Error(
                `About to discontinue ${potentialDiscontinued.length} products. ` +
...
async function cleanseUnmatchedProductsJob(token, { overrideFailsafe = false } = {}) {
    addLog('--- Starting Unmatched Product Cleanup Job with Validation ---', 'error');
